...
test("cursor scopes are distinct per mailbox and per company", () => {
  const a = triageCursorScope("company-1", "personal");
  const b = triageCursorScope("company-1", "support");
  const c = triageCursorScope("company-2", "personal");
  ...
  // Must not collide with the poll loop's own cursors.
  assert.notEqual(a.namespace, "imap");
});
