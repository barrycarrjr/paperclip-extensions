 ...
 * managed plugin directory and the source folder is no longer needed at
 * runtime.
 *
 * The `createRequire` banner on the worker bundle is required because some
 * runtime deps (nodemailer, googleapis, etc.) are CJS and use dynamic
 * `require()` calls that don't resolve in plain ESM. The banner polyfills
 * `require` against the worker's import.meta.url so the bundled CJS code
 * works at runtime.
 */
