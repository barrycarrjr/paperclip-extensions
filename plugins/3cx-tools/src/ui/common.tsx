/**
 * Shared layout primitives for the Phone-section pages. Kept small so
 * every list-view page has the same header / table / empty-state / error
 * affordances without each one re-implementing them.
 */
...
export function PageContainer({ children }: { children: ReactNode }) {
  return (
    <div style={{ padding: 16, color: "var(--foreground)" }}>{children}</div>
  );
}
...
export function Badge({
  ...
  return (
    <span style={{ ...badgeBase, ...toneStyles[tone] }}>{children}</span>
  );
}
