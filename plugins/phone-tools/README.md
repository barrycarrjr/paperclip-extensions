...
- **v0.7.11** — Patch bump alongside the cross-plugin release. No functional changes; ensures the Plugin Manager surfaces the update so installed copies stay current with the registry.

- **v0.7.10** — Patch bump alongside the cross-plugin release. No functional changes; ensures the Plugin Manager surfaces the update so installed copies stay current with the registry.

- **v0.7.9** — Patch bump alongside the cross-plugin release. No functional changes; ensures the Plugin Manager surfaces the update so installed copies stay current with the registry.

- **v0.7.8** — Verified personal caller IDs API via Twilio's OutgoingCallerIds (list/start-verification/delete) so agents can place calls from a verified personal number. Per-call reason substitution with a universal preflight rule plus a dedicated Gather Info task. Assistant editing UI in the Agent Phone tab.

- **v0.7.7** — Fix the outbound-call failure mode where LLM agents would invent inline `assistant` configs in `phone_call_make` calls instead of using the saved Vapi assistant projection. v0.7.6 surfaced this on PER-113: Alex drafted a phone call to a seller, the operator approved, the approval flow re-dispatched the tool correctly — but Vapi rejected the call because the inline assistant config was missing the engine-side fields the saved Vapi assistant already has (`voice`, model, tools like endCall / addToDnc / transferCall, the safety/DNC/transfer preambles). The Recent calls list showed no call ever going out, the LLM mis-diagnosed the symptom as "approval didn't auto-execute," and re-triggered the tool with a fresh idempotencyKey — looking like an approval loop. Two-part fix: (1) tool-side auto-swap in `phone_call_make` — when the calling agent already has a saved `vapiAssistantId` in PhoneConfig AND the LLM passed an inline `assistant` config, swap to using the saved assistant id and inject the inline config's `firstMessage` + `systemPrompt` as per-call overrides (firstMessageOverride + systemPromptOverride, with safety/DNC/transfer preambles re-wrapped around the override prompt so the per-call swap doesn't lose those rails). The LLM's per-call intent (its tailored opening, its task-specific reference info) is preserved while the engine-side machinery comes from the saved assistant. (2) prompt-side guidance in `composeAssistant` — the wizard-generated systemPrompt now explicitly instructs agents: do NOT pass an inline `assistant` config to `phone_call_make`, omit the param and the plugin uses your saved Vapi projection automatically; use `reason` for the spoken opening line; calls require approval, do not re-trigger with different idempotencyKeys while a draft is pending.

- **v0.7.6** — Outbound call image attachments + per-call reference context + cross-agent delegation hook. The "Have <agent> call someone" modal now accepts paste / drag-drop / file-upload of images. Attached photos get fed to the host's new `ctx.ai.complete` SDK surface (plugin declares the new `ai.complete` capability), which routes them through whatever vision-capable provider the operator has configured — including Claude Code via the claude_local adapter, which materializes the bytes to files in a per-call workspace and reads them with its native Read tool. Extracted facts flow to a new `additionalContext` body field on the place-call route, which the route handler folds into a per-call `systemPromptOverride` (system message only, NOT firstMessage — see `assistantOverrides.model.messages` in the Vapi engine), so the assistant has the info as silent reference instead of reciting model numbers and prices on the call. Wizard-generated `callingStyle` block gains two long-needed bullets that close common outbound-call drift: (a) "you initiated this call, you are the inquiring party, they are NOT your customer — don't ask if they have questions for you," and (b) "close the call directly once you have the info; no 'anything else?'-style fishing." On `setPhoneConfig` save, the plugin now declares a `phone` capability for the agent at the host's well-known plugin-state convention (`scope=agent, namespace="capabilities", state_key="phone"`) so other agents can discover this assistant via the new `paperclipFindAgentsByCapability` MCP tool and delegate phone work via child issues. New plugin route `assistants.calls.describe-attachments` (POST) backs the image-extraction step. UX polish: Recent calls list shows per-row timestamps and each row is clickable (re-uses TestCallModal as a transcript viewer); fixed a long-standing transcript bug where the modal read `transcript.turns` but the API returns `transcript.structured` (per NormalizedTranscript) — added a 30s grace polling window after the call terminates so Vapi has time to flush `artifact.messages` before we stop polling, and the empty-state copy now distinguishes still-in-flight vs. flushing vs. no-transcript-produced. Vapi engine: `StartCallInput.systemPromptOverride` triggers a pre-call assistant GET so we can replay the saved model provider/model/tools/temperature alongside the new messages array — the override no longer accidentally strips endCall/transferCall/addToDnc tools. Vendors paperclip's SDK as `vendor/sdk/{plugin-sdk,shared}.tgz` consumed via `pnpm.overrides` so the plugin can resolve `ctx.ai.complete` against Barry's forked SDK without an npm publish.

- **v0.7.5** — Verified personal caller IDs API via Twilio's OutgoingCallerIds (list/start-verification/delete) so agents can place calls from a verified personal number. Per-call reason substitution with a universal preflight rule plus a dedicated Gather Info task. Assistant editing UI in the Agent Phone tab.

- **v0.7.4** — Preflight "check what you need before acting" block added to wizard-generated assistants' systemPrompt. Before placing any call the assistant must audit required info (phone number, target, specifics); if anything required is missing or ambiguous it posts one comment listing every gap, reassigns the issue to the reporter, and waits — instead of dialing the wrong number or making up details. Batches all questions in one round so the reporter does one reply, not three. Mirrors the cross-cutting rule landing in Paperclip's default AGENTS.md so wizard-built phone agents inherit the same discipline at the source, not from a stale copy. Two new test assertions covering both EA and custom assistant types.

- **v0.7.3** — Per-call `{the reason for call}` substitution. The placeholder in the wizard-generated firstMessage is now a true **runtime template** rather than something the operator was expected to edit before saving (v0.7.2 framed it wrong). `phone_call_make` accepts a new optional `reason` string (e.g. `"getting a price on a golf cart"`); the tool reads the calling agent's saved firstMessage from PhoneConfig, substitutes the placeholder with the reason, and ships the result as a new `StartCallInput.firstMessageOverride` field that both engines apply per-call (Vapi via `assistantOverrides.firstMessage`; DIY by overriding `state.assistant.firstMessage` before the first turn). UI-initiated calls flow through the same path — the test-call endpoint strips the placeholder sentence (no reason), and the place-call endpoint substitutes with the existing `objective` body field. If no reason is passed and the saved firstMessage still contains the placeholder, the entire sentence is stripped so the engine never speaks the literal token. The wizard's first-message textarea is preserved as an operator-customizable override (useful for fixed-greeting agents), but the surrounding copy now correctly explains the placeholder is auto-substituted by the engine. Tests: 6 new assertions covering `substituteCallReason` (provided reason, whitespace trimming, missing reason drops the clause, empty-string handled like missing, non-templated firstMessage left unchanged, integration with `composeAssistant`). All compose tests pass.

- **v0.7.2** — Wizard-generated assistant prompt overhaul. The EA template now includes a fifth task type — **Gather information** — with detailed how-to guidance (ask specific questions, take detailed notes, confirm ambiguities, deliver findings back). The system prompt's calling-style block gains a **no-name-leak rule**: agents introduce themselves by name only ("Alex, an assistant calling on behalf of a client") and don't volunteer the operator's name unless asked, because most call recipients don't recognize it and leading with it sounds odd. Voicemail rule simplified — no longer instructs the agent to "give a callback number" (the engine handles callback routing separately). The wizard's first-message rendering changed from a read-only preview to an **editable textarea**: the composed default contains a `{the reason for call}` placeholder the operator is expected to replace with the assistant's actual purpose before saving, since the engine speaks firstMessage verbatim. A yellow warning surfaces if any `{...}` placeholder remains at save time. Tests: 6 new assertions covering the new task, no-name-leak rule, voicemail copy, and intro-line phrasing; updated firstMessage and trimming assertions for the new format. 112/112 pass.

- **v0.7.1** — Setup-tab documentation in the manifest now covers all v0.5.x features across 11 sectioned walkthroughs — warm transfer, campaigns, federal DNC, audit export, predictive pacing, portfolio rollup — with time-to-complete and required-vs-optional indicators. Closes the gap where the in-Paperclip Setup tab only documented v0.1.0 onboarding. Pure docs change.

- **v0.7.0** — AI Calls navigation IA. The standalone Assistants + Campaigns sidebar entries are gone; in their place is a single collapsible **🎙️ AI Calls** group that nests Assistants, Campaigns, and three new operator-facing pages: **Inbound routes** (DID → assistant mapping; read-only today, mutation surface lands in v0.7.x), **DNC list** (account-local DNC entries + federal DNC cache status), and **Audit log** (every campaign dial decision; filterable by decision + date range with one-click CSV export). Three new data channels (`phone.inbound-routes`, `phone.dnc-list`, `phone.audit-log`) back the pages — they wrap existing engine reads + plugin-state queries. Sibling of the 3cx-tools Phone group; the two render side-by-side in the sidebar rail so the operator's full telephony surface reads as one logical area. Pages share a `ui/common.tsx` primitive set with 3cx-tools' equivalent for visual consistency. Sidebar collapsed-state persists in `localStorage`.
...
- **v0.6.1** — DIY engine: per-call conversation state now persists via `ctx.state` instead of in-memory only. A worker restart mid-call (e.g. during a 5-minute LLM thread) no longer loses the conversation history; the next Jambonz hook can read state from where the previous worker left off. Storage keys: `instance::diy:call:<accountKey>:<callSid>` for the full DiyCallState, `instance::diy:index:<accountKey>` for a per-account summary index used by `phone_call_list`. Index is capped at 500 entries per account — oldest terminal entries are evicted automatically on upsert so storage can't grow unbounded. `gc()` method drops terminal entries older than a configurable threshold for explicit cleanup. Six new tests cover cross-engine state recovery, append-turn persistence, list-sorting, drop, gc, and per-account namespace isolation. Total suite: 103/103 pass.

- **v0.6.0** — DIY engine. The `engine: "diy"` slot is no longer an `[EENGINE_NOT_AVAILABLE]` stub: it now runs against a self-hosted Jambonz install plus a provider-neutral LLM (Anthropic or OpenAI) via plain HTTP. Jambonz handles SIP / RTP / STT / TTS; the plugin composes Jambonz verb sequences (`say` / `gather`) and calls the LLM between turns. No Vapi dependency, no $0.05/min orchestration markup, audio and transcripts stay on infrastructure you control. New per-account config: `jambonzApiUrl`, `jambonzApiKeyRef`, `jambonzAccountSid`, `jambonzApplicationSid`, `hostBaseUrl`, `diyLlmProvider` (anthropic/openai), `diyLlmApiKeyRef`, `diyLlmModel`, `diyTtsVendor`/`diyTtsVoice`/`diyTtsLanguage`, `diySttVendor`/`diySttLanguage` — all `showWhen: engine="diy"` so Vapi accounts don't see them. New apiRoutes `/diy/jambonz/{call,next,status}` handle Jambonz application hooks with HMAC signature verification (auth: "webhook"). Scope of this slice: outbound calls work end-to-end with mocked-Jambonz tests (verb composition, conversation turns, LLM call, hangup heuristics, signature verification — 18 tests). Inbound and saved-assistant CRUD return `[ENOT_SUPPORTED]` on DIY and land in v0.6.x. **Operator setup**: stand up Jambonz, create an application pointing its call-webhook at `https://<paperclip-host>/api/plugins/phone-tools/api/diy/jambonz/call`, configure STT (Deepgram) + TTS (Google / ElevenLabs) credentials on the Jambonz side, then fill the new fields on the plugin settings page. Vapi engine path is unchanged.
